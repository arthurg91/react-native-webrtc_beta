...
public class WebRTCModule extends ReactContextBaseJavaModule {
    ...
    @ReactMethod
    public void mediaStreamTrackSetWithSpecificReactTagEnabled(String id, boolean enabled, String reactTag) {
        ThreadUtils.runOnExecutor(() -> mediaStreamTrackWithSpecificReactTagSetEnabledAsync(id, enabled, reactTag));
    }

    private void mediaStreamTrackWithSpecificReactTagSetEnabledAsync(String id, boolean enabled, String reactTag) {
        MediaStreamTrack track = getTrackWithReactTag(id, reactTag);
        if (track == null) {
            Log.d(TAG, "mediaStreamTrackSetEnabled() track is null");
            return;
        } else if (track.enabled() == enabled) {
    ...
    private MediaStreamTrack getTrackWithReactTag(String trackId, String streamReactTag) {
        MediaStreamTrack track = getLocalTrack(trackId);
        if (track == null) {
            for (int i = 0, size = mPeerConnectionObservers.size(); i < size; i++) {
                PeerConnectionObserver pco = mPeerConnectionObservers.valueAt(i);
