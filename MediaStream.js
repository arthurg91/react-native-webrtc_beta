...
export default class MediaStream extends EventTarget(MEDIA_STREAM_EVENTS) {
  ...
  enableAudioTracks(enabled: boolean) {
    this.getAudioTracks().forEach(track => track.enableWithReactTag(enabled, this._reactTag));
  }

  enableVideoTracks(enabled: boolean) {
    this.getVideoTracks().forEach(track => track.enableWithReactTag(enabled, this._reactTag));
  }
